...
*/
let allHrsArray = Array.from(allHrs)
// Set the border of the all the hr elements to ""
allHrsArray.map(elm => elm.style.border = "1px solid tomato")
// Change the background of all the hr to "" using for of loop.
allHrsArray.map(elm => elm.style.backgroundColor = "antiquewhite")
// Change the 'border-radius' of all the hr to "5px" using array.
allHrsArray.map(elm => elm.style.borderRadius = "5px")
// Change the alignment of the heading(h1) to center.
heading.style.textAlign = `center`
// Change the font size of the heading to 3rem.
heading.style.fontSize = `3rem`
// Change the border of hr with class 'image' to `2px solid purple`.
let hrImg = document.querySelectorAll('hr.image').forEach(elem => elem.style.border = "2px solid purple");
// Hide the box number 17 (last box).
document.querySelector('.seventeen').style.display = 'none';
// Change the border of all the hr element from solid to dashed type
allHrsArray.map(elem => elem.style.borderStyle = "dashed");
...
p.innerText = "querySelector returns an element you can maupulate but querySelectorAll returns the collection of elements in array kind of structure.";
